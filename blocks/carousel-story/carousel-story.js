...
export function showSlide(block, slideIndex = 0, behavior = 'smooth') {
  ...
  block.querySelector('.carousel-story-slides').scrollTo({ top: 0, left: activeSlide.offsetLeft, behavior });
}
...
function bindEvents(block) {
  ...
  block.querySelector('.slide-prev').addEventListener('click', () => { showSlide(block, parseInt(block.dataset.activeSlide, 10) - 1); });
  block.querySelector('.slide-next').addEventListener('click', () => { showSlide(block, parseInt(block.dataset.activeSlide, 10) + 1); });
  const slideObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry) => { if (entry.isIntersecting) updateActiveSlide(entry.target); });
  }, { threshold: 0.5 });
  block.querySelectorAll('.carousel-story-slide').forEach((slide) => { slideObserver.observe(slide); });
}
